...
;(function($) {
    ...
    var  mobileSize = 982; // size control for change images
	...
	$.fn.touchSlider = function(options){
		...
		var options = $.extend({
			...
			onChange: function(){
                var position = $('.item.active').attr('data-item');
                
                $('[data-slider-position]').removeClass('active');
                
                $('[data-slider-position="'+position+'"]').addClass('active'); 
            },
			onStart: null,
			onCheckItems: null,
			lockScroll: false,
		...
		this.each(function() {
            ...
            if(options.StartAuto && options.PauseOnHover){
                pauseSlider();                
            }
            ...
            function changeImages(){
                var slide_img       = $('.gallery-holder img');
                var slide_content   = $('.gallery-holder .sliderContent');
                var slide_list      = $('.gallery-holder .list');
                
                if(width <= mobileSize){ //mobile
                   ...
                   //ajustar de container de texto no slide 
                   slide_content.css({'width': width, 'margin-left': '0'});
                   slide_list.css('transform', 'none');
                }else{ //desktop
                    slide_img.width(2000); //slide_img.width('100%');
                    
                    slide_img.each(function( index ) {
                      $(this).attr('src', $(this).attr('data-img-desktop'));
                    });
                    
                    //cálculo para centralizar container de texto no slide 
                    var container_width = $('.container').width();
                    var item_width      = '2000';//$('.item').width();
                    var left = ( parseInt(item_width)/2 ) - ( parseInt(container_width)/2 );
                    
                    slide_content.css({'width': container_width, 'margin-left': left});
                }
                delayResizeItems();
            }
            
            function changeSliderContainer(){
            ...
            function autoSlider(){
                StartAuto = setInterval(
                    function(){ 
                        var totalSliders = parseInt($('.item').length);

                        var  currentSlider = $('.item.active').attr('data-item');
                        var moveTo = parseInt(currentSlider) + 1;

                        if( moveTo < totalSliders){
                            moveToIndex(moveTo);
                        }else{
                            moveToIndex(0);
                        }
            ...
            function pauseSlider(){
                var PauseSlider = $('.item');
                
                PauseSlider.mouseenter(function(){
                   clearInterval(StartAuto);
                });
                
                PauseSlider.mouseleave(function(){
                   autoSlider();
                });   
            }
